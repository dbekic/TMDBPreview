...
public extension UIScrollView {
    /// Checks if the current position of scroll view is near bottom with given offset.
...
public extension Reactive where Base: UIScrollView {
    ...
    /// It is useful in case when you have pull to refresh where you need to reset what was the latest position on bottom of scroll view.
    ///
    ...
    func reachedBottomOnceWith(restart: Driver<Void>, offset: CGFloat = 200.0) -> Driver<Void> {
        restart
            .startWith(())
            .flatMapLatest { [unowned base] in
                base.rx.reachedBottomOnce(offset: offset)
            }
    }
